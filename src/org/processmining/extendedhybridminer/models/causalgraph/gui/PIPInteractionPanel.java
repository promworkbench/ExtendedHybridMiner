...
public class PIPInteractionPanel extends JPanel implements MouseListener, MouseMotionListener, ViewInteractionPanel {
	...
	public static final int PIPSIZE = 500;
	...
	private final HybridGraphVisualization panel;
	//private JComponent component;
	private ScalableComponent scalable;
	//private ScalableComponent originalScalable;
	//private double scale;

	public PIPInteractionPanel(HybridGraphVisualization panel) {
	...
	public synchronized void initializeImage() {
		Dimension size = scalable.getComponent().getPreferredSize();
		if (size.getWidth() > 0 && size.getHeight() > 0) {
			...
			BufferedImage image = (BufferedImage) scalable.getComponent().createImage(getWidth(), getHeight());
			Graphics2D g2ds = image.createGraphics();

			g2ds.setColor(scalable.getComponent().getBackground());
            ...
            //Rectangle rect =((ProMJGraph) scalable).computeVisibleRect();
			/*DirectedGraph graph = ((ProMJGraph) scalable).getProMGraph();
			GraphLayoutConnection layoutConnection=new GraphLayoutConnection(graph);
			ProMGraphModel model=new ProMGraphModel(graph);
			this.localScalable = new ProMJGraph(model, new ViewSpecificAttributeMap(), layoutConnection);
			this.localScalable.setScale(r * oldScale);
			this.component = this.localScalable.getComponent();*/
			scalable.getComponent().paint(g2ds);
			scalable.setScale(oldScale);
			g2ds.dispose();

			parentScroll.setViewPosition(p);
			//parentScroll.setViewPosition(new Point(rec.xPos, rec.yPos));
			//drawMain(rec.getX(), rec.getY());
			setRect();
	...
	/*private void updateScalable() {
		ProMJGraph graph = ((ProMJGraph) this.originalScalable);
		GraphLayoutConnection con = new GraphLayoutConnection(graph.getProMGraph());
		ProMGraphModel model = new ProMGraphModel(graph.getProMGraph());
		this.scalable = new ProMJGraph(model, true, graph.getViewSpecificAttributes(), con);
	}*/

	public double getVisWidth() {
		return scalable.getComponent().getSize().getWidth();
	}

	public double getVisHeight() {
		return scalable.getComponent().getSize().getHeight();
	}
	...
	public double getHeightInView() {
		...
		double ratio = size.getHeight() / (size.getWidth() + size.getHeight());
	...
	public double getWidthInView() {
		...
		double ratio = size.getWidth() / (size.getWidth() + size.getHeight());
		return PIPSIZE*ratio;
	}
	...
	public void reset(ScalableComponent scalable) {
		this.removeMouseMotionListener(this);
		this.removeMouseListener(this);
		this.removeAll();
		this.addMouseMotionListener(this);
		this.addMouseListener(this);
		this.setScalableComponent(scalable);
	}

	/*public void setOriginalGraph(ScalableComponent s) {
		this.originalScalable = s;
	}*/
