 ...
 */
public class PlacePWSetStruct<N extends AbstractDirectedGraphNode> {
	private Set<PlaceEvaluation<N>> currentSet;	
	private PlacePWSetStruct<N> nested;
	private PlacePWSetStruct<N> sibling;
	private CurrentDepth currentDepth;
	private boolean isRoot;


	// To be used for the "root" of the recursive struct.
	public PlacePWSetStruct() {
	...
	public PlacePWSetStruct(CurrentDepth currentDepth, Set<PlaceEvaluation<N>> currentSet) {
	...
	private void increaseDepthRec(PlaceEvaluation<N> ppe) {
		...
		if (this.level()<this.currentDepth.depth && ppe.isDisjoint(this.currentSet)) {
			...
			// nested==null
			if (this.nested==null) {
				this.nested = new PlacePWSetStruct<N>(this.currentDepth, set);
			}
			// nested!=null navigate at the end of nested and add the new set at the last position.
			else {
				PlacePWSetStruct<N> lastSiblingOfNextLevel = this.nested.getLastSibling();
				lastSiblingOfNextLevel.sibling = new PlacePWSetStruct<N>(currentDepth, set);
			}
	...
	public boolean isConflicting(PartialPlaceEvaluation<N> ppe) {
		...
		System.out.println("---- DEPTH "+this.currentDepth.depth+" Checking conflict for "+ppe+" and "+this.currentSet);
...
/*		if (this.nested!=null)
	...
	public void addPlace(PlaceEvaluation<N> p) {
		...
		// if no more sibling and I am at the first level, I need to add the singleton {p} to the siblings.
			...
			if (this.level()==1) {
				...
				singletonSet.add(p);
				this.sibling = new PlacePWSetStruct<N>(currentDepth, singletonSet);
			}
		}
	}


	private PlacePWSetStruct<N> getLastSibling() {
		if (this.sibling==null)
			return this;

		PlacePWSetStruct<N> result = this.sibling;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		PlacePWSetStruct other = (PlacePWSetStruct) obj;
	...
	public PlacePWSetStruct<N> getNested() {
		return nested;
	}


	public PlacePWSetStruct<N> getSibling() {
		return sibling;
	}
	...
	class CurrentDepth {
		...
		private PlacePWSetStruct<N> getOuterType() {
			return PlacePWSetStruct.this;
		}
